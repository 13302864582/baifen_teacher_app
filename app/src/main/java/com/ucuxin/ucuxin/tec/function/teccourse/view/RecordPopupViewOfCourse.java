...
import com.ucuxin.ucuxin.tec.function.teccourse.AddHandoutActivity.DialogListener;
...
public class RecordPopupViewOfCourse extends FrameLayout implements OnClickListener/*, OnTouchListener */{
	...
	private void setupViews() {
		View view = LayoutInflater.from(getContext()).inflate(R.layout.voice_course_popup_window, null);
		...
		case R.id.add_voice_answer_container:
			...
			if (visibility == View.VISIBLE) {
				...
				if (textStr.equals(mActivity.getString(R.string.click_to_record_text))) {//点击开始录音
					...
					isRecording = true;
					MediaUtil.getInstance(false).record(mAudioName, mCallback, mActivity , this ,MaxRecordTime.MAX_OF_COURSE);
				}
			}else {
				playVoice();
			}
...
//			mAddAnswerVoiceContainer.setOnTouchListener(RecordPopupViewOfCourse.this);
	...
	private void playVoice() {
		...
		MediaUtil.getInstance(false).playVoice(true, mAudioPath, mAnimationDrawable,
